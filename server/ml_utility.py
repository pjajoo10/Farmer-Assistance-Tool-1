...
class DiseasePrediction:
    def __init__(self):
        ...
        self.model.load_state_dict(torch.load("./MLmodels/disease-prediction.pth", map_location=torch.device('cpu')))
        self.model.eval()

    def resize_image(self, image: Image, length=256) -> Image:
    ...
    def predict_disease(self, path="./uploads/test.jpg"):
        convert_tensor = transforms.ToTensor()
        img = Image.open(path)

        # resize image and convert to tensor for model
        resized_img = self.resize_image(img)
        # print(resized_img.size)
        img_tensor = convert_tensor(resized_img)
